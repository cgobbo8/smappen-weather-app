...
type ForecastUiWithIcon = ForecastUi & {mostFrequentIcon: string, minTempHue: number, maxTempHue: number, minTempHslClass: string, maxTempHslClass: string};

const getDayShort = (date: string) => new Date(date).toLocaleDateString('en-US', {weekday: 'short'});
const getMostUsedIcon = (icons: string[]) => Array.from(new Set(icons)).reduce((prev, curr) =>
    icons.filter(el => el === curr).length > icons.filter(el => el === prev).length ? curr : prev
);
...
export const useCityStore = defineStore('city', () => {
  ...
  const dispatchSelectCity = async (city: City) => {
    ...
    const forecastDaysWithIconList = forecastWeather.list.reduce((prev, curr) => {
      const correspondingItemByDay = prev.find(forecast => getDayShort(forecast.date) === getDayShort(curr.dt_txt));
      ...
      const updatedCorrespondingItem = {
        ...
        icons:  curr.weather?.[0]?.icon ? [...correspondingItemByDay.icons, curr.weather?.[0]?.icon] : [...correspondingItemByDay.icons]
      }
  ...
  dispatchLatLonChange(ToulouseLatLng).catch(err => new Error(err))


  return { selectedCity, loadedCities,currentTime, forecastNextHours, forecastNextDays, currentLatLon, cleanCities, dispatchLatLonChange, dispatchSelectCity, dispatchLoadCities }
})
